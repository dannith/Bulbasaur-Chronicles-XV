...
class Bulbasaur(pygame.sprite.Sprite):
    ...
    def __init__(self, name):
        ...
        self.name = text.render(name, True, (0, 0, 0))
        self.rotation = 180
        self.locX = 100
        self.locY = 100
        textW = self.name.get_width()
        self.textLoc = self.locX - (textW - 22) / 2
    ...
    def getSprite(self):
        if self.rotation > 338 or self.rotation <= 23: self.image = self.frames["side"]
        elif self.rotation > 23 and self.rotation <= 68: self.image = self.frames["sideFront"]
        elif self.rotation > 68 and self.rotation <= 113: self.image = self.frames["front"]
        elif self.rotation > 113 and self.rotation <= 158: self.image = self.frames["sideFront"]
        elif self.rotation > 158 and self.rotation <= 203: self.image = self.frames["side"]
        elif self.rotation > 203 and self.rotation <= 248: self.image = self.frames["sideBack"]
        elif self.rotation > 248 and self.rotation <= 293: self.image = self.frames["back"]
        elif self.rotation > 293 and self.rotation <= 338: self.image = self.frames["sideBack"]
        return self.image
    ...
    def update(self):
        ...
        self.rect.y = self.locY
        textW = self.name.get_width()
        self.textLoc = 1 + self.locX - (textW - 22) / 2
        screen.blit(self.name, (self.textLoc, self.locY - 10))
        screen.blit(self.render, self.loc)
